...
.. note::
   The following documentation describes a custom script for generating a navigation drawer and associated elements.

Nav Drawer
----------

The "Nav Drawer" section defines a navigation drawer with circular elements and lines. This is typically used for creating a menu or navigation panel in a graphical user interface.

- ``cirs x, y, radius, color``: Draws a circle at the specified position (x, y) with the given radius and color.
- ``line x1, y1, x2, y2, color``: Draws a line from point (x1, y1) to point (x2, y2) with the specified color.
- ``fill x, y, width, height, color``: Fills a rectangular area starting at (x, y) with the specified width, height, and color.
...
- Similar to the Nav Drawer, it uses circle, line, and fill commands to create shapes and lines.
...
- Inner Container 1 and Inner Container 2 use similar drawing commands as the Nav Drawer and Main Container.
...
- Circular elements and lines are used to create button shapes.
- The ``fill`` command is used to fill the buttons with color.

Nav Drawer Icons
----------------

This section defines icons within the navigation drawer, and their visibility depends on the value of the ``fondo`` variable.

- Icons are displayed based on the selected theme (``fondo`` value), with different positions and images.
...
- The displayed text content varies based on the selected system language (``sys0``).
...
Script Flow and Custom Variables
---------------------------------
...
- Custom variables like ``button.val`` and ``fontColor.val`` are used to control the script's behavior and appearance.

This script appears to be designed for creating a graphical user interface, possibly for a touchscreen application or embedded system. Modifications and customizations can be made by adjusting the parameters and conditions within the script.
...
.. code-block:: javascript
    ...
    {
      //Spanish
      xstr 295,25,260,25,4,fontColor.val,color,1,1,3,"Presión final"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Valor de alarma y de advertencia presión final (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancelar"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==1)
    {
      //Italian
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Pressione finale"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Valore di allarme e avviso pressione finale (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Annulla"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==2)
    {
      //French
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Pression ultime"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Valeur d'alarme et d'avertissement pression finale (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Annuler"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==3)
    {
      //English
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Final pressure"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Alarm and warning value final pressure (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancel"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==4)
    {
      //German
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Enddruck"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Alarm- und Warnwert Enddruck (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Stornieren"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==5)
    {
      //Portuguese
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Pressão final"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Pressão final do valor de alarme e aviso (bar)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancelar"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }
    //Page text end
