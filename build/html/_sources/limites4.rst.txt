...
This script defines the layout and elements of a graphical user interface (GUI). It includes the creation of a navigation drawer, main container, inner containers, buttons, navigation drawer icons, page title, page text, and a number keyboard. Each section of the script is explained below.

Navigation Drawer
----------------

This section begins the definition of the navigation drawer, which typically contains icons or navigation options. It includes:
...
Here, the main content area of the GUI is defined, including:

- Two circular elements (`cirs`) within the main container.
- Four lines (`line`) outlining the main container.
- Three filled rectangles (`fill`) to provide background color to specific regions within the container.

Inner Containers
----------------

This script defines two inner containers, each with similar structures, comprising:
...
The script defines the appearance of two buttons: a Cancel Button and an OK Button. Each button includes:
...
This section dynamically displays icons based on the value of the `fondo` variable. The icons change to reflect different themes. It includes conditional checks and the `pic` command to display theme-specific icons.
...
The script defines a numeric keypad, with digits 1 through 9, '0', and 'x'. It includes:

- Circular elements (`cirs`) and lines (`line`) to create buttons for each digit.
- Text labels (`xstr`) for each digit.
- Special buttons for '0' and 'x'.

Conclusion
----------

This script is designed to create a GUI with various elements, such as buttons, icons, text, and containers. The layout and appearance of these elements are defined with specific coordinates, sizes, colors, and conditions, making it suitable for interactive applications.
...
.. code-block:: javascript
    ...
    {
      //Spanish
      xstr 295,25,260,25,4,fontColor.val,color,1,1,3,"Temperatura del aceite"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Valor de alarma y de advertencia temperatura del aceite (°C)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancelar"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==1)
    {
      //Italian
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Temperatura dell'olio"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Valor de alarma y de advertencia temperatura del aceite (°C)"
    ...
    {
      //French
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Température de l'huile"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Alarme de température d'huile et valeur d'avertissement (°C)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Annuler"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==3)
    {
      //English
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Oil temperature"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Oil temperature alarm and warning value (°C)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancel"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==4)
    {
      //German
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Öltemperatur"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Öltemperatur Alarm- und Warnwert (°C)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Stornieren"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }else if(sys0==5)
    {
      //Portuguese
      xstr 325,25,200,25,4,fontColor.val,color,1,1,3,"Temperatura do óleo"
      xstr 160,110,280,35,0,fontColor.val,45347,0,1,3,"Alarme de temperatura do óleo e valor de advertência (°C)"
      xstr 270,265,60,25,2,0x3B44,65535,1,1,3,"Cancelar"
      xstr 370,265,60,25,2,WHITE,0x3B44,1,1,3,"OK"
    }
    //Page text end
